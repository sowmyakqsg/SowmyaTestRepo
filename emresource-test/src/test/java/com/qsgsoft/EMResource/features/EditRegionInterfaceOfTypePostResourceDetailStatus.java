...
' Requirement         :Edit Region Interface Of Type 'PostResourceDetailStatus'
' Date		          :20-Oct-2012
' Author	          :QSG
'-----------------------------------------------------------------------------
...
public class EditRegionInterfaceOfTypePostResourceDetailStatus {
	Date gdtStartDate;
	ReadData rdExcel;
	static Logger log4j = Logger
			.getLogger("com.qsgsoft.EMResource.features.EditRegionInterfaceOfTypePostResourceDetailStatus");
	static {
...
/***************************************************************
'Description		:Verify that a region interface of type 'Get Hospital Status' can be edited.
'Precondition		:
'Arguments		:None
'Returns		:None
'Date			:10/25/2012
...
***************************************************************/
	...
	@Test
	public void testBQS101823() throws Exception {
		boolean blnLogin = false;
		String strFuncResult = "";
		Login objLogin = new Login();// object of class Login
		Interface objInterface = new Interface();
		try {
			gstrTCID = "101823"; // Test Case Id
			gstrTO = " Verify that a region interface of type 'Post Resource " +
					"Detail Status' can be edited.";
			gstrReason = "";
			gstrResult = "FAIL";
			...
			/*
			 * STEP : Action:Preconditions: Interface 'Test Post Resource Detail
			 * Status' of type 'Post Resource Detail Status' is created in
			...
			try {
				assertEquals("", strFuncResult);
				strFuncResult = objInterface.navToInterfaceList(seleniumPrecondition);
			} catch (AssertionError Ae) {
				gstrReason = strFuncResult;
			}
			...
			try {
				assertEquals("", strFuncResult);
				strFuncResult = objInterface
						.navToSelInterfaceTypePage(seleniumPrecondition);
			} catch (AssertionError Ae) {
				gstrReason = strFuncResult;
			}
			...
			try {
				assertEquals("", strFuncResult);
				strFuncResult = objInterface.navCreateNewInterfPage(seleniumPrecondition,
						strType);
			} catch (AssertionError Ae) {
				gstrReason = strFuncResult;
			}
			try {
				...
				String strResId = "Use the EMResource Resource ID";
				strFuncResult = objInterface.fillCreatRgnInterfaceFlds(
						seleniumPrecondition, strInterfaceName, strDesc, strContactInfo,
						strResId, false, true);
			} catch (AssertionError Ae) {
				gstrReason = strFuncResult;
			}
			...
			try {
				assertEquals("", strFuncResult);
				strFuncResult = objInterface.savAndVerifyInterface(seleniumPrecondition,
						strInterfaceName);
			} catch (AssertionError Ae) {
				gstrReason = strFuncResult;
			}
			 ...
			 * associated with 'Test Post Resource Detail Status' interface.
			 * Expected Result:'Edit region Interface' screen is displayed. Data
			 ...
			 * screen is displayed. Interface 'Test Post Resource Detail Status'
			 * is displayed under it with the updated values under appropriate
			 * columns.
			 */
			// 588970
			try {
			...
			gstrTCID = "BQS-101823";
			gstrTO = "Verify that a region interface of type 'Post Resource Detail Status' can be edited.";
			gstrResult = "FAIL";
			log4j.info(e);
			log4j.info("========== Test Case '" + gstrTCID
					+ "' has FAILED ==========");
