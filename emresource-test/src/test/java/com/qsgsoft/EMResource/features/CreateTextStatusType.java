...
/**********************************************************************
' Description :This class includes test cases to Create Text Status types
' Precondition:
' Functions: 
' Date		  :03-May-2012
' Author	  :QSG
...
' <Date>                           	         <Name>
'*******************************************************************/
public class CreateTextStatusType {
	...
	************************************************************************/
	...
	@Test
	public void testBQS83388()throws Exception{
		try{
			gstrTCID = "83388";			
			 ...
			 * Step 4: Select 'Text' and click on 'Next' <-> 'Create Text Status Type' screen is displayed.
			...
			String strData[]={"edit",statTypeName,"Text","No","","Uncategorized",strStatTypDefn,""};
			try {
				assertTrue(strFailMsg.equals(""));
				//check status type is displayed in Status Type List screen
	 ...
	 /***********************************************************************
		 ...
		 ************************************************************************/
	@Test
	public void testBQS68660() throws Exception {
		...
		try {
			...
			gstrTO = "Verify that the option to trace the user"
					+ " can be selected for a text status type.";
			gstrReason = "";
			gstrResult = "FAIL";
			...
			try {
				...
				strUpdatTxtValue = strErrorMsg[0];

				if (strUpdatTxtValue.equals("Auto")
						|| strUpdatTxtValue.equals("--")) {
					strUpdatTxtValue = "EAuto";
				} else {
					strUpdatTxtValue = "DAuto";
				}
			...
			try {
				...
				strFuncResult = objViews.fillAndSavUpdateStatusTST(selenium,
						strUpdatTxtValue, strSTvalue[0], true, "",
						"");
			...
			try {
				...
				strFuncResult = objViews.fillAndSavUpdateStatusTST(selenium,
						strUpdatTxtValue, strSTvalue[0], true, strUserName,
						strNwPassword);
			...
			try {
				...
				strFuncResult = objViews.fillAndSavUpdateStatusTST(selenium,
						strUpdatTxtValue, strSTvalue[0], true, strIncUserName,
						strPassword);
			...
			gstrTO = "Verify that the option to trace the"
					+ " user can be selected for a text status type.";
			gstrResult = "FAIL";
			excReason = null;
