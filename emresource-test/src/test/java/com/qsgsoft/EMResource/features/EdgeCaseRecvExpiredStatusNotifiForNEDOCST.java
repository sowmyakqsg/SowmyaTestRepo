...
/*************************************************************************
 * ' Description       :This class includes ' testcases 'from requirement
 * ' Requirement Group :Setting up Status types
 * ' Requirement       :Receive expired status notifications for NEDOCS ST
 * ' Date              :3rd-July-2013
 * ' Author            :QSG
 * '----------------------------------------------------------------------
 ...
 **************************************************************************/
 ...
 public class EdgeCaseRecvExpiredStatusNotifiForNEDOCST {

	Date gdtStartDate;
	ReadData rdExcel;
	static Logger log4j = Logger.getLogger("com.qsgsoft.EMResource.features."
			+ "EdgeCaseRecvExpiredStatusNotifiForNEDOCST");
	static {
	...
	//start//testBQS119737//
	/****************************************************************************************************
	'Description	:Verify that user receives the following when the NEDOCS status of a resource expires
	                 at the EXPIRATION time: 1. Status update prompt. 2. Expired status notification
	'Precondition	:
	'Arguments		:None
	'Returns		:None
	'Date			:7/2/2013
	...
	*********************************************************************************/
	@Test
	public void testEdgeCase119737() throws Exception {
		...
		try {
			gstrTCID = "BQS-119737";
			gstrTO = "Verify that user receives the following when the NEDOCS status of a resource expires at the"
					+ " EXPIRATION time:"
			...
			String strStatusTypeValue = "NEDOCS Calculation";
			String statTypeName = "NEDOC" + strTimeText;
			...
			String strUpdateValue1 = "135 - Overcrowded";
			String strUpdateValue2 = "241 - Disaster";
				...
				try {
					...
					try {
						...
						strFuncResult = objViews.fillUpdateNEDOCSTAndSave(
								selenium, strScUpdValue1, strSTvalue[0], false);
					} catch (AssertionError Ae) {
						gstrReason = strFuncResult;
					}
	...
	*****************************************************************************************************/
	...
	@Test
	public void testEdgeCase119738() throws Exception {
		boolean blnLogin = false;
		Login objLogin = new Login();// object of class Login
		String strFuncResult = "";
		General objMail = new General();
		...
		Roles objRole = new Roles();
		try {
			gstrTCID = "119738"; //Test Case Id
			gstrTO = " Verify that user receives the following when the NEDOCS status of a resource expires at the SHIFT time:"
					+ "1. Status update prompt."
					+ "2. Expired status notification";//Test Objective
			gstrReason = "";
			gstrResult = "FAIL";
			...
			String strUpdateValue1 = "241 - Disaster";
			String strUpdateValue2 = "135 - Overcrowded";
			String strScUpdValue1[] = { "1", "2", "3", "4", "5", "6", "7" };
			String strScUpdValue2[] = { "0", "1", "2", "3", "4", "5", "6" };
			...
			// Status Type
			String strStatusTypeValue = "NEDOCS Calculation";
			String statTypeName = "AutoNedoc_" + strTimeText;
			String strStatTypDefn = "Auto";
			...
			/*
			 * STEP : Action:Verify that user receives the following when the
			 * NEDOCS status of a resource expires at the SHIFT time: 1. Status
			 * update prompt. 2. Expired status notification Expected Result:No
			 * Expected Result
			 */
			// 654512
			strFuncResult = objLogin.login(seleniumPrecondition,
			...
			try {
				...
				try {
					...
					try {
						assertEquals("", strFuncResult);
						strFuncResult = objViews.fillUpdateNEDOCSTAndSave(
								selenium, strScUpdValue2, strSTvalue[0], true);
					} catch (AssertionError Ae) {
						gstrReason = strFuncResult;
					}
				} catch (AssertionError Ae) {
