...
' Requirement Group   :Create Region Interface GetHospitalStatus
...
public class CreateRgnInterfaceGetHospitalStatus {
	
	Date gdtStartDate;
	ReadData rdExcel;
	static Logger log4j = Logger
			.getLogger("com.qsgsoft.EMResource.features.CreateRgnInterfaceGetHospitalStatus");
	static{
...
/********************************************************************************
'Description	:Verify that a region interface of type 'Get Hospital Status' can be created.
'Precondition	:None
'Arguments		:None
'Returns		:None
'Date	 		:22-Oct-2012
...
**********************************************************************************/
	@Test
	public void testBQS94041() throws Exception {
		...
		try {
			gstrTCID = "94041";
			gstrTO = "Verify that a region interface of type 'Get Hospital Status' can be created.";
			gstrResult = "FAIL";
			gstrReason = "";
			...
			String strWebServiceAction="getHospitalStatus";
			String strType="Get Hospital Status";
			String strSchema="EDXL-HAVE / Hospital Status draft/pre 1.0";
			String strDesc="Automation";
			String strContactInfo="QSG";
			...
			/*1 	Preconditions:
				...
				No other active interface is present in the region with the interface type 'Get EDXL-HAVE 1.0' 		No Expected Result 
				*/
			...
			/*3 	Navigate to Setup >> Interfaces 		'Interface List' screen is displayed.
			...
			try {
				...
				strFuncResult = objInterface.checkIntergaceDetailsInIntListPage(
						selenium, interfaceNameExixting, strActiveExisting,
			...
			/*4 	Click on 'Create New Interface' button 		'Select Interface Type' screen is displayed with:
			...
			try {
				assertEquals("", strFuncResult);
				strFuncResult = objInterface.navToSelInterfaceTypePage(selenium);
			} catch (AssertionError Ae) {
				gstrReason = strFuncResult;
			}
			...
			try {
				assertEquals("", strFuncResult);
				String strElementID="css=input[name='actionName'][value='"+strWebServiceAction+"']";
			...
			try {
				assertEquals("", strFuncResult);
				log4j
						.info("Under web service Action field '"+strWebServiceAction+"' is displayed by default. ");
				...
				strFuncResult = "Under web service Action field '"+strWebServiceAction+"' is NOT displayed by default. ";
				gstrReason = strFuncResult;
				log4j
						.info("Under web service Action field '"+strWebServiceAction+"' is NOT displayed by default. ");
			}
			...
			try {
				...
				String strElementID = "//div[@id='mainContainer']/form/table/tbody/tr/td[text()='Custom Status Type Mappings']";
				strFuncResult = objGeneral.checkForAnElements(selenium,
						strElementID);
				...
				gstrReason = strFuncResult;
				log4j
						.info("'Optional Status Type Mappings' and "
			...
			try {
				assertEquals("", strFuncResult);
				log4j
						.info("'Custom status Type Mappings' sections are displayed "
								+ "with list of status types (Number status types) and "
								+ "check box corresponding to each status types. ");

			
			} catch (AssertionError Ae) {
				strFuncResult = "'Custom status Type Mappings' sections is NOT displayed "
						+ "with list of status types (Number status types) and "
						+ "check box corresponding to each status types. ";
				gstrReason = strFuncResult;
				log4j
						.info("'Custom status Type Mappings' sections is NOT displayed "
								+ "with list of status types (Number status types) and "
								+ "check box corresponding to each status types. ");
			}
			
			/*6 	Fill the mandatory data and select Active check box click on save. 		'Interface' is created and listed in 'Interface List' screen with appropriate values under all the columns.
