...
/**********************************************************************
' Description :This class includes DeleteStatusChangePreferences requirement 
'				testcases
' Precondition:
' Date		  :20-Oct-2012
' Author	  :QSG
...
public class DeleteStatusChangePreferences  {
	Date gdtStartDate;
	ReadData rdExcel;
	static Logger log4j = Logger
			.getLogger("com.qsgsoft.EMResource.features.DeleteStatusChangePreferences");
	static{
	...
	'Precondition	:1. Test user has created status types 'NST', 'MST' & 'SST'
	...
	**********************************************************************************/
	...
	@Test
	public void testBQS70187() throws Exception {
		...
		/*	8. Following values are provided for user 'A' under 'Status Change Preferences'
			...
			*/
			...
			/*3 	Provide the following values:
				1. NST: A value above 'A'
				2. MST: Select status 'S2'
				3. SST: A value below 'Y' 		User A receives the following:
				1. 'Email', 'Pager' and 'Web' for 'NST'
				2. 'Email', 'Pager' and 'Web' for 'MST'
				3. 'Email', 'Pager' and 'Web' for 'SST' 
				*/
			...
			try {
				...
				strFuncResult = objViews.fillAndSavUpdateStatusMST(selenium,
						strStatusValue[0], strSTvalue[2], false, "", "");
				selenium.click("css=input[value='Save']");
				selenium.waitForPageToLoad(gstrTimeOut);
			} catch (AssertionError Ae) {
				gstrReason = strFuncResult;
			...
			try {
				assertEquals("", strFuncResult);
				strFuncResult = objViews.verifyUpdateSTValInViews(selenium,
						statrMultiTypeName, strStatusName1, "3");
			} catch (AssertionError Ae) {
				gstrReason = strFuncResult;
			}
			...
			try {
				...
				String strDesc = "On " + strCurrDate + " " + strResource
						+ " changed " + statrNumTypeName + " status from 101 to 102.";
			...
			try {
				...
				strMsgBody2 = statrNumTypeName + " from 101 to 102; "
						+ "Reasons: \nSummary at " + strCurrDate + " \n"
						+ "Region: " + strRegn + "";
			...
			try {
				...
				String strDesc = "On " + strCurrDate + " " + strResource
						+ " changed " + statrSaturatTypeName + " status from 429 to 323.";
			...
			try {
				...
				strMsgBody2 = statrSaturatTypeName + " from 429 to 323; "
						+ "Reasons: \nSummary at " + strCurrDate + " \n"
						+ "Region: " + strRegn + "";
...
//MST
			try {
				...
				String strDesc = "On " + strCurrDate + " " + strResource
						+ " changed " + statrMultiTypeName + " status from "
			...
			try {
				...
				strMsgBody2 = statrMultiTypeName + " from " + strStatusName2
						+ " to " + strStatusName1 + "; "
						+ "Reasons: \nSummary at " + strCurrDate + " \n"
						+ "Region: " + strRegn + "";
			...
			if(intResCntNotRecv==12&&intResCnt==6){
				gstrResult = "PASS";
			}
