...
' Date					Name
'*******************************************************************/
public class EditNumberStatusType {

	Date gdtStartDate;
	ReadData rdExcel;
	static Logger log4j = Logger
			.getLogger("com.qsgsoft.EMResource.features.EditNumberStatusType");
	static {
	...
	@Test
	public void testBQS98833() throws Exception {
		...
		try {
			...
			String strStatTypDefn = "Autof";

			String statEdTypeName = "AEd_" + strTimeText;
			String strEdStatTypDefn = "AutoEdf";
			...
			try {
				assertTrue(strFuncResult.equals(""));
				strFuncResult = objEve.navToEventManagementNew(seleniumPrecondition);
			} catch (AssertionError Ae) {
				gstrReason = strFuncResult;
			}
			try {
			...
			try {
				assertTrue(strFuncResult.equals(""));
				strFuncResult = objEve.navToEventManagementNew(selenium);
			} catch (AssertionError Ae) {
				gstrReason = strFuncResult;
			}
			...
			try {
				assertEquals("", strFuncResult);
				String strArST[] = { statEdTypeName };
				String strArRs[] = { strResource };
				strFuncResult = objPref.verifyRTSTAndRSInCustView(selenium,
						strResrctTypName, strArRs, strArST);
			} catch (AssertionError Ae) {
				gstrReason = strFuncResult;
			}
			try {
