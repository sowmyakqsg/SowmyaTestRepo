...
/**********************************************************************************************
 * ' Description  :This class includes Receive expired status notifications for TST' testcases '
 * ' Precondition : 
 * ' Date         :12-June-2012
 * ' Author       :QSG
 * '------------------------------------------------------------------------------------------ '
...
public class RecvExpiredStatusNotifiForTST {
	Date gdtStartDate;
	ReadData rdExcel;
	static Logger log4j = Logger
			.getLogger("com.qsgsoft.EMResource.features.RecvExpiredStatusNotifiForTST");
	static {
	...
	/**************************************************************************************************
	'Description	:Verify that user receives the following when the text status of a resource expires 
	                 at the SHIFT time:   1. Status update prompt.  2. Expired status notification
	'Precondition	:None
	'Arguments		:None
	'Returns		:None
	'Date	 		:12-June-2012
	...
	****************************************************************************************************/
	...
	@Test
	public void testBQS69020() throws Exception {
		...
		try {
			gstrTCID = "BQS-69020 ";
			gstrTO = "Verify that user receives the following when the text status "
					+ "of a resource expires at the SHIFT time:1. Status update prompt.2."
			...
			try {
				...
				strFuncResult = objViews.verifyUpdateST(selenium,
						strResrctTypName, statTypeName, strUpdateValue);
			...
			gstrTCID = "BQS-69020";
			gstrTO = "Verify that user receives the following"
					+ " when the text status of a resource expires "
					+ "at the SHIFT time:1. Status update prompt.2."
					+ " Expired status notification";
			gstrResult = "FAIL";
			log4j.info(e);
			log4j.info("========== Test Case '" + gstrTCID
	...
	****************************************************************************************************/
	...
	@Test
	public void testBQS69018() throws Exception {
		try {
			gstrTCID = "BQS-69018";
			gstrTO = "Verify that user receives the following when the text"
					+ " status of a resource expires at the EXPIRATION time:"
			...
			String strUpdateValue1 = "s";
			String strUpdateValue2 = "t";
			String strStatusTypeValue = "Text";
			String statTypeName = "AutoTSt_" + strTimeText;
			String strStatTypDefn = "Auto";
			...
			String strViewName = "AutoV_" + strTimeText;
			String strVewDescription = "";
			String strViewType = "Resource (Resources and status types as "
					+ "rows. Status, comments and timestamps as columns.)";
			...
			log4j.info("~~~~~TEST CASE - " + gstrTCID	+ " EXECUTION STARTS~~~~~");
					...
					try {
						...
						strFuncResult = objViews.fillAndSavUpdateStatus(
								selenium, strUpdateValue1, strSTvalue[0],
								false, "", "");
					} catch (AssertionError Ae) {
						gstrReason = strFuncResult;
					}
					...
					try {
						...
						try {
							assertEquals("", strFuncResult);
							strFuncResult = objViews.fillAndSavUpdateStatus(
									selenium, strUpdateValue2, strSTvalue[0],
									false, "", "");
						} catch (AssertionError Ae) {
							gstrReason = gstrReason + " " + strFuncResult;
						}
			...
			gstrTCID = "BQS-69018";
			gstrTO = "Verify that user receives the following when the text status of a resource expires at the EXPIRATION time:"
					+ "1. Status update prompt."
					+ "2. Expired status notification";
			gstrResult = "FAIL";
			log4j.info(e);
