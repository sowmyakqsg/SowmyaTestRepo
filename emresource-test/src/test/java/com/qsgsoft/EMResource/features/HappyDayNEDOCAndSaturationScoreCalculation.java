...
public class HappyDayNEDOCAndSaturationScoreCalculation {

	Date dtStartDate;
	ReadData rdExcel;
	static Logger log4j = Logger
			.getLogger("com.qsgsoft.EMResource.features.HappyDay_NEDOCAndSaturationScoreCalculation");
	static {
	...
	*****************************************************************************************/
	...
	@Test
	public void testHapyDay120528() throws Exception {
		...
		try {
			gstrTCID = "120528"; // Test Case Id
			gstrTO = "Verify that scores for 'Saturation score' and 'NEDOC' is "
					+ "calculated only when clicked on 'Calculate' button";// TO
			gstrReason = "";
			gstrResult = "FAIL";
	        ...
	        String strNedocValue1="241 - Disaster";
	        String strSatUpdValues[] = { "1", "2", "3", "4", "5", "6", "7","8", "9" };
			String strSatValue = "429%";
			String strSatValue1 = "429";
		...
		log4j.info("~~~~~PRE-CONDITION" + gstrTCID + " EXECUTION STARTS~~~~~");
			...
			try {
				assertEquals("", strFuncResult);
				strFuncResult = objStatusTypes.selectStatusTypesAndFilMandFlds(
						seleniumPrecondition, strSatScoreValue, statrSatStatTypeName,
						strStatTypDefn, true);
			} catch (AssertionError Ae) {
				gstrReason = strFuncResult;
			}
		...
		log4j.info("~~~~~TEST CASE " + gstrTCID + " EXECUTION STARTS~~~~~");
			...
			try {
				assertEquals("", strFuncResult);
				strFuncResult = objViews.fillAndSavUpdateNEDOCAndVerifyValue(
						selenium, strNedocUpdValues, strSTvalue[0],
						strNedocValue, false);
			} catch (AssertionError Ae) {
				gstrReason = strFuncResult;
			}
			try {
				assertEquals("", strFuncResult);
				strFuncResult = objViews.fillAndSavUpdateSSTAndVerifyValue(
						selenium, strSatUpdValues, strSTvalue[1], strSatValue,
						true);
			} catch (AssertionError Ae) {
				gstrReason = strFuncResult;
			}
			try {
				...
				strFuncResult = objViews.verifyUpdateSTValInViews(selenium,
						statrNEDOCStatTypeName, strNedocValue1, "3");
			} catch (AssertionError Ae) {
				gstrReason = strFuncResult;
			}
			try {
				assertEquals("", strFuncResult);
				strFuncResult = objViews.verifyUpdateSTValInViews(selenium,
						statrSatStatTypeName, strSatValue1, "4");
			} catch (AssertionError Ae) {
				gstrReason = strFuncResult;
			}
