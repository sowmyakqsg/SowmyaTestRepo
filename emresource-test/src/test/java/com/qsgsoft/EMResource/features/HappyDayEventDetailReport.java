...
public class HappyDayEventDetailReport {
  
	Date dtStartDate;      
	ReadData rdExcel;
	static Logger log4j = Logger
			.getLogger("com.qsgsoft.EMResource.features.HappyDay_EventDetailReport");
	static {
	...
	****************************************************************************************/	
	@Test
	public void testHapyDay119793() throws Exception {		
		String strFuncResult="";
		boolean blnLogin=false;		
		Paths_Properties objAP = new Paths_Properties();//object of class Paths_Properties
		Properties pathProps = objAP.Read_FilePath();//object of class Paths_Properties
		...
		try {
			...
			String strNSTValue = "NEDOCS Calculation";
			...
			*/
			...
			try {
				...
				String[] strStatusTypeValue = {strSTvalue[0] };
			...
			try {
				...
				String strArStatType2[] = {  statNumTypeName };
			...
			try {
				...
				strFuncResult = objViews.verifyUpdateSTValInViews(selenium,
						statNumTypeName, "241 - Disaster", "3");
			} catch (AssertionError Ae) {
				gstrReason = strFuncResult;
			}	
			...
			try {
				...
				if (f.isFile()) {
					...
					objOFC.writeResultData(strTestData, strWriteFilePath,
							"Eventdetailreport");
					gstrResult = "PASS";
					System.out.println("Generated PDF  is saved in the specified folder ");
				}
				} catch (AssertionError Ae) {
				gstrReason = strFuncResult;
