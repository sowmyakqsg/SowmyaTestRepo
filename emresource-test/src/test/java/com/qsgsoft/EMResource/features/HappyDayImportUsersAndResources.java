...
' Date					                               Name
'*******************************************************************/
public class HappyDayImportUsersAndResources {

	Date dtStartDate;
	ReadData rdExcel;
	static Logger log4j = Logger
			.getLogger("com.qsgsoft.EMResource.features.HappyDay_ImportUsersAndResources");
	static {
    ...
    *************************************************************************************/
	...
	/***************************************************************************************************
	'Description :Verify that user can download the Upload Template by clicking on 'Upload Template' link
	'Arguments	 :None
	'Returns	 :None
	'Date		 :16/July/2013
	'Author		 :QSG
	...
	****************************************************************************************************/
	@Test
	public void testHapyDay123388() throws Exception {
		...
		try {
			gstrTCID = "123388"; // Test Case Id
			gstrTO = "Verify that user can download the Upload Template by clicking" +
					" on 'Upload Template' link";// TO
			gstrReason = "";
			gstrResult = "FAIL";
		  ...
		  log4j.info("~~~~~TEST-CASE" + gstrTCID + " EXECUTION STATRTS~~~~~");
			...
			try {
				...
				try {
					...
					log4j
							.info("Following fields are NOT displayed with appropriate value and "
									+ "is disabled.1. Spreadsheet: File path ");
			...
			try {
				...
				String strUploadFilePath =pathProps
						...
						+ "Up_Temp" + gstrTCID + strTimeText + ".xls";
				strFuncResult = objUpload.fillNewUploadFields(selenium,
						strAutoFilePath, strUploadFilePath, strAutoFileName, false,
						true);
			} catch (AssertionError Ae) {
				gstrReason = strFuncResult;
			}
			...
			try {
				assertEquals("", strFuncResult);
				strFuncResult = objUpload.navToEditUserPageByClickingOnUser(
						selenium, strUserID);
			} catch (AssertionError Ae) {
				gstrReason = strFuncResult;
			}
			...
			try {
				assertEquals("", strFuncResult);
				// search user data
				String strByRole = rdExcel.readInfoExcel("User_Template", 7,
						11, strFILE_PATH);
				String strByResourceType = rdExcel.readInfoExcel(
						"User_Template", 7, 12, strFILE_PATH);
				String strByUserInfo = rdExcel.readInfoExcel("User_Template",
						7, 13, strFILE_PATH);
				String strNameFormat = rdExcel.readInfoExcel("User_Template",
						7, 14, strFILE_PATH);
			...
			gstrTCID = "123388"; // Test Case Id
			gstrTO = "Verify that user can download the Upload Template by clicking on 'Upload Template' link";// TO
			gstrResult = "FAIL";
			log4j.info(e);
